...
### 2026-09-24 — Recuperación de historial de `production` (Fase W1): deploy sin force-push + 404 real para assets

Contexto: el HEAD de `production` que Plesk tenía registrado (`5db7e29`) quedó huérfano (root commit sin padres) porque cada deploy hacía `git init` + `git push --force` sobre un repo efímero en `dist/`, descartando toda la historia anterior en cada corrida. Plesk reportaba `fatal: bad object HEAD (Error code 128)` al intentar resolver ese HEAD. Plan de recuperación de varias fases (`plesk-git-recovery`); esta entrada cubre solo la Fase W1, los cambios en `main` que deben aterrizar ANTES de reconstruir `production`.

- **`.github/workflows/deploy.yml` reescrito.** Ya no crea un repo git efímero por corrida. Ahora clona el branch `production` existente (`--branch production --single-branch --depth 1`), y solo en la primera corrida (branch inexistente) cae a `checkout --orphan production` + `git rm -rf .`. El contenido de `publish/` se reemplaza por completo (`find ... -exec rm -rf` + `cp -a dist/. publish/`) para que los assets con hash viejo se BORREN en vez de acumularse, y `git add -A` captura esas eliminaciones. Si no hay diff contra el commit anterior (`git diff --cached --quiet`), el step sale limpio sin commitear. El push final ya NO usa `--force` — un rechazo ahora es una señal real de carrera, no algo a silenciar. Se agrega `concurrency: { group: publish-production, cancel-in-progress: false }` porque sin `--force` dos corridas simultáneas podrían chocar.
- **`public/web.config`: nueva regla "Static assets bypass SPA fallback".** Insertada antes de la regla "SPA fallback" existente, con `match url="^(assets|documents|media)/"` y `action type="None"` + `stopProcessing="true"`. Antes, un archivo faltante bajo `/assets/`, `/documents/` o `/media/` caía en la regla SPA fallback y el server respondía `200 text/html` (el navegador rechaza `text/html` para un `<script type="module">`, y un PDF/video roto se ve como si "cargara" pero es HTML). Ahora esos tres prefijos devuelven un 404 real cuando el archivo no existe, sin afectar el resto de las reglas (`staticContent`, `mimeMaps`, headers, `<location path="assets">` con cache de 1 año siguen intactos).
- Alcance extendido de `^assets/` a `^(assets|documents|media)/` con aprobación explícita del usuario: mismo defecto de fondo, cubre también los PDFs de Requisitos/Leyes y el video de Distancias.
- Verificación: `npx tsc -b` limpio, `npm run build` genera `dist/` sin errores y `dist/web.config` contiene la nueva regla (Vite copia `public/` tal cual a `dist/`). Ambos `web.config` (`public/` y `dist/`) son XML bien formado.
- Fuera de alcance de esta entrada (fases posteriores, gated por aprobación del usuario): validar el workflow reescrito contra CI real (Fase W2), tag de backup de `production` (Fase W4), el graft que re-parenta `production` sobre `5db7e29` con `git commit-tree` (Fase W5, requiere aprobación explícita antes del push), y el pull en el panel de Plesk + purge de Cloudflare + verificación en vivo (Fase W6).
